...
public class BitBuffer implements Cloneable, IntEncoder {
     ...
     *
     * @return A newly allocated array of characters.
     */
    public char[] decodeChars() {

	//
	// Read the number of bytes in the array of characters.
	int utflen;
	if(!pop()) {
	    pop();
	    utflen = directDecode(8);
	} else {
	    if(!pop()) {
		utflen = directDecode(16);
	    } else {
		utflen = directDecode(32);
	    }
	}
	...
	str.ensureCapacity(utflen);
        int c, char2, char3;
	int count = 0;

	//
	// Convert encoded integers to strings.
	while (count < utflen) {
     	    c = directDecode(8);
	    switch (c >> 4) {
		...
		/* 0xxxxxxx*/
		count++;
		str.a[str.len++] = (char) c;
		break;
	    case 12: case 13:
		/* 110x xxxx   10xx xxxx*/
		count += 2;
		char2 = directDecode(8);
		str.a[str.len++] = (char)(((c & 0x1F) << 6) | (char2 & 0x3F));
		break;
	    case 14:
		/* 1110 xxxx  10xx xxxx  10xx xxxx */
		count += 3;
		char2 = directDecode(8);
		char3 = directDecode(8);
		str.a[str.len++] = (char)(((c & 0x0F) << 12) |
					  ((char2 & 0x3F) << 6) |
					  ((char3 & 0x3F) << 0));
		break;
	    }
     ...
     */
    public String decodeUTF() {
	char[] a = decodeChars();

	//
	// An empty array of characters means we need to return the empty
	// string.
	if(a == null) {
	    return "";
	}
