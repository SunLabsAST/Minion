  ...
  public class WordEntry{
	...
	String wdstring = wd.wordstring;
      ...
      if (LexiconUtil.startsLikeNumber(wdstring)){
         try {numericalValue = new Double(wdstring);
        ...
        if (Character.isLetter(wdstring.charAt(wdstring.length()-1))){
          ...
          if (wdstring.indexOf(".") < 0) { //there's no decimal point
             ...
             if ((numericalValue.doubleValue() > Integer.MAX_VALUE) ||
                 ...
                 this.putdict(this.lexicon.numValErrorAtom, this.lexicon.trueAtom);
             else
...
// waw 02-18-05:
                form = this.lexicon.innerMakeWord(wdstring.substring(i+1,j), wd.morphCache);
                ...
                if (tailstring.length() > 1){
                    ...
                    if (jplus > 0) qstr = qstr.substring(0,jplus);
                    qual = this.lexicon.makeSenseWordIfNeeded(cat, qstr);
                }
            }else {
//              form = innerMakeWord(wdstring.substring(i+1)); // waw 02-18-05:
              form = this.lexicon.innerMakeWord(wdstring.substring(i+1), wd.morphCache);
            }
            sensenamep = new Object[]{cat, form, tailstring};
          }
          else
             Lexicon.log.debug(Lexicon.logTag, 10, "**** warning: unknown category in " +
          ...
          if (form != null){  // beefed up pmartin 18apr02
              this.putdict(this.lexicon.senseofAtom, new List(this.lexicon, form));
              form.addSubsense(wd);
...
//              if (!(form.isNonnameFormOfCat(cat))) form.addWordCategory(cat, 0);
         ...
         if (wdstring.indexOf("_") > -1)
             components = this.lexicon.makeWordSet(wdstring, "_", wd.morphCache);
         else if (wdstring.indexOf("-") > -1)
             components = this.lexicon.makeWordSet(wdstring, "-", wd.morphCache);
         if (components != null)
      ...
      if (!wipeall && (properties != null)){
        ...
        while (propEnum.hasMoreElements()) {
            ...
            if (this.lexicon.salvageProperties.contains(akey)){
                if (savedProps == null)
                    savedProps = new ConcurrentHashMap(Lexicon.PROPERTIES_HASH_SIZE);
    ...
    // tweaked to add rootCategory info and fix bad rootCat .. pmartin 29jun01
      ...
      if (newCat.isInArray(noPenaltyCategories)){
          if (penalty == 0){ return noPenaltyCategories;
          }else{
              noPenaltyCategories =
          ...
          if (penalty == 1){ return penalty1Categories;
        }else{
          penalty1Categories =
          LexiconUtil.removeArrayElement(newCat, penalty1Categories);
        }
      } else if (newCat.isInArray(penalty2Categories)){
          if (penalty == 2) {return penalty2Categories;
        }else{
          penalty2Categories =
          LexiconUtil.removeArrayElement(newCat, penalty2Categories);
        }
      } else if (newCat.isInArray(penalty3Categories)){
          if (penalty == 3) {return penalty3Categories;
        }else{
          penalty3Categories =
          LexiconUtil.removeArrayElement(newCat, penalty3Categories);
        }
      }
      switch (penalty) {
      case 0:
    ...
    public void fixIcatProperties( Word myWord, boolean pppast){
        ...
        if (tenseV == null) tenseV = getdict(this.lexicon.tnsAtom);
...
//                            "\ntenseV = " + tenseVStr);
        ...
        if ((tenseV != null) && (tenseV.listp())){
            ...
            if ((LexiconUtil.isInList(this.lexicon.pastpartAtom, formL)) ||
                ...
                if (pppast) mergeIcat(this.lexicon.verbCategory, this.lexicon.pastCategory);
            ...
            if (LexiconUtil.isInList(this.lexicon.presentAtom, tenseL)){
                if ((LexiconUtil.isInList(this.lexicon.threeSAtom, agrL)) ||
                    ...
                    mergeIcat(this.lexicon.verbCategory, this.lexicon.v3sgCategory);
                    ...
                    mergeIcat(this.lexicon.verbCategory, this.lexicon.v2sgCategory);
                    ...
                    mergeIcat(this.lexicon.verbCategory, this.lexicon.v1sgCategory);
                    ...
                    mergeIcat(this.lexicon.verbCategory, this.lexicon.v13sgCategory);
                    ...
                    mergeIcat(this.lexicon.verbCategory, this.lexicon.not13sgCategory);
                    remdict(this.lexicon.not13SgAtom);
                }else mergeIcat(this.lexicon.verbCategory, this.lexicon.not3sgCategory);
            }
        }
    ...
    public int getCatPenalty (Category c){ //pmartin 17may01
       if (c.subsumesOneOf(noPenaltyCategories)) return 0;
       else if (c.subsumesOneOf(penalty1Categories)) return 1;
       else if (c.subsumesOneOf(penalty2Categories))  return 2;
       else if (c.subsumesOneOf(penalty3Categories)) return 3;
       else return 4; // flag for bogus penalty
    }
    ...
    */
    ...
    public boolean isNonnameFormOfCat (Category c) {
      if (!isFormOfCat(c)) return false;
      if (!this.lexicon.nnCategory.subsumesCategory(c)) return true;
      if (isExplicitCat(this.lexicon.nounCategory)) return true; // pm 16apr04
      if (isNonNounFormOfCat(c)) return true; // pm 26apr04
      if (!isFormOfCat(this.lexicon.nnCategory)) return false;
      return true;
    }

    public boolean isNonNounFormOfCat (Category c) { //pm 26apr04
      Category[] nonNounCats = c.collectExcluding(this.lexicon.nounCategory);
      if (nonNounCats == null) return false;
      for (int i=0; i < nonNounCats.length; i++)
          if (isFormOfCat(nonNounCats[i])) return true;
      return false;
    }
    ...
    public boolean isNonnameFormOfCat (Category[] cc) {
      if (cc != null) {  //pmartin added null test 14sep99
        for (int i=0; i < cc.length; i++) {
          if (isNonnameFormOfCat(cc[i])) return true;
        }
      }
      return false;
    }
   ...
   //  System.out.print ("rootcat["+ i+ "]=" + this.rootCategories[i] + ", ");
       if (c.subsumesOneOf(this.rootCategories)) return true;
       //tru("flunkedRootCats ");
      ...
      if (value != null){
        if (addToList) {
          if (value.listp()) {
            ...
            return getdict(p);
          }else return addtodictprop(p, value, true);
        }else return putdict(p, value);
      }else  return getdict(p);
    }
    ...
    */
    public Value markDict (Atom p, Value[] valArray, boolean addToList) {
      if (valArray!= null){
        if (addToList) {
          ...
          return getdict(p);
        } else return putdict(p, this.lexicon.makeList(valArray));
      }else
        return getdict(p);
    }
      ...
      if (properties != null) {
        ...
        while (it.hasNext()) {
          ...
          if (propVal instanceof List){
            propValString =  LexiconUtil.safePrintString(((List)propVal).contents);
          } else {
    ...
    /* addtodictprop adds property values to property lists of words */
     ...
     public Value addtodictprop (Atom p, Value value, boolean atEnd) {
        ...
        if (value == null)
        ...
        }else if (oldval.equal(value)) return oldval;
            ...
            if (Lexicon.authorFlag)
                if (Lexicon.debug)
                     Lexicon.log.debug(Lexicon.logTag, 10, "this=" + this +
            ...
            for (int ii=0; ii < li; ii++)  array[ii+offset]=oldarray[ii];
            return putdict(p, this.lexicon.makeList(array));
          }
    }
   ...
   //                          " and val " + newval);
     ...
     if (p == null)
        Lexicon.log.debug(Lexicon.logTag, 10, "**** warning: null attribute in putdict " //+
                          // wordstring
                          + " for value: " + newval);
     else if (newval == null)
        Lexicon.log.debug(Lexicon.logTag, 10, "**** warning: null value in putdict " //+
                          // wordstring
                          + " for property: " + p);
     else properties.put(p, newval);
     return newval;
    }
