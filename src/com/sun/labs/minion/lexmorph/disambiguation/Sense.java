...
public class Sense implements Serializable {
    ...
    transient private Log log = Log.getLog();
    
    transient private static String logTag = "SENSE";
    ...
    public String toString() {
        ...
        List<Map.Entry<String,Float>> temp = new ArrayList<Map.Entry<String,Float>>();
        while(pq.size() > 0) {
            temp.add(pq.poll());
        }
        ...
        for(Iterator<Map.Entry<String,Float>> i =  temp.iterator(); i.hasNext(); ) {
            ...
            sb.append(String.format("%s %f", e.getKey(), lm.logToLinear(e.getValue())));
            if(i.hasNext()) {
                sb.append(", ");
            }
        }
