...
public class LexiconUtil {
	...
	// static method for testing intersection of a value (maybe list) with array:
	  ...
	  public static boolean valueArrayIntersectp (Value val1, Value[] list2) {
	      if ((val1 == null) || (list2 == null)) return false;
	      else if (val1.listp()) return intersectp(((List)val1).contents, list2);
	      else return (memb(val1, list2) > 0);
	  }
	...
	public static synchronized String safePrintString (Value[] array) {
	    ...
	    while (i < array.length) {
	      ...
	      if (array[i] == null)
	        val = val + "null";
	      else val = val + array[i].safePrintString();
	      i++;
	    }
	    return val;
	  }
	 ...
	 // tweaked 25jul02 pmartin for multiples of remove element
	  public static Value[] removeArrayElement (Value elt, Value[] array) {
	    if (isMembOfArray(elt, array)){
	      ...
	      for (int ii=0; ii < len; ii++)
	        if (array[ii] != elt) newArray[j++] = array[ii];
	      return newArray;
	    }else {
	      return array;
	    }
	...
	// static method for printing contents of arrays of values:
	  ...
	  public static synchronized String printString (Value[] array) {
	    ...
	    while (i < array.length) {
	      ...
	      if (array[i] == null)
	        val = val + "null";
	      else val = val + array[i].printString();
	      i++;
	    }
	    return val;
	  }
	...
	// tweaked 16oct01 to use a Word[] to accumulate instead of vector
	  public static Word[] mergeArrays(Word[] wa1, Word[] wa2){
	        if ((wa1 == null) || (wa1.length == 0)) return wa2;
	        else if ((wa2 == null) || (wa2.length == 0)) return wa1;
	        ...
	        if (newIdx == 0) return wa1; // all were duplicates
	
	        int oldLen = wa1.length;
	        Word[] wa3 = new Word[oldLen+newIdx];
	        for(int i=0; i < oldLen; i++) wa3[i] = wa1[i];
	        for(int i=0; i < newIdx; i++) wa3[oldLen+i] = newWa2[i];
	        return wa3;
	  }
	...
	public static Category[] mergeArrays(Category[] ca1, Category[] ca2){
	        if ((ca1 == null) || (ca1.length == 0)) return ca2;
	        else if ((ca2 == null) || (ca2.length == 0)) return ca1;
	...
	public static boolean isMemberOfArray (Value elt, Value[] array) {
	    if ((elt != null) && (array != null)){  //pmartin null test added 14sep99
	      int i = 0;
	      while (i < array.length) {
	        if (array[i].equal(elt)) return true;
	        i++;
	      }
	    }
	    return false;
	...
	// static method for testing intersection of arrays of values:
	  ...
	  public static boolean intersectp (Value[] list1, Value[] list2) {
	      if ((list1 == null) || (list2 == null)) return false; //pmartin 25may01
	    int i, j;
	    i = 0;
	...
	public static String fringeString(Value[] vals){//pmartin 14may01
	     ...
	     for (int i=0; i < vals.length; i++){
	         ...
	         else frg = val.printString(); // covers Atom, Categories, Words
	         if (fringeBuf.length() != 0) fringeBuf.append("_");
	...
	public static String fringeString(List lval){
	        if (lval == null) return "";
	        else return fringeString(lval.contents);
	  }
	...
	// public static boolean wordTrace = true;
	  ...
	  public static Word[] concatArrays(Word[] wa1, Word[] wa2){
	                                 // pmartin 11mar04
	        int w1 = 0;
	        if (wa1 != null) w1 = wa1.length;
	        int w2 = 0;
	        if (wa2 != null) w2 = wa2.length;
	        int w3 = w1 + w2;
	        if (w3 == 0) return null;
	        Word[] newwa = new Word[w3];
	        for(int i=0; i < w1; i++) newwa[i] = wa1[i];
	        for(int i=0; i < w2; i++) newwa[w1+i] = wa2[i];
	        return newwa;
	  }
	...
	public static Value[] concatArrays(Value[] va1, Value[] va2){
	                                 // pmartin 11mar04
	        int v1 = 0;
	        if (va1 != null) v1 = va1.length;
	        int v2 = 0;
	        if (va2 != null) v2 = va2.length;
	        int v3 = v1 + v2;
	        if (v3 == 0) return null;
	        Value[] newva = new Value[v3];
	        for(int i=0; i < v1; i++) newva[i] = va1[i];
	        for(int i=0; i < v2; i++) newva[v1+i] = va2[i];
	        return newva;
	  }
	...
	public static Value[] addValueToArray(Value[] va, Value newV){
	        Value[] newVa;
	        if ((newV == null) || (isMemberOfArray(newV, va))) return va;
	        else {
	            int len = 0;
	            if (va != null) len = va.length;
	            newVa = new Value[len+1];
	            newVa[len] = newV;
	            for (int i=0; i < len; i++) newVa[i] = va[i];
	        }
	        return newVa;
	  }
	...
	// static methods  to add element to array or merge two arrays
	  ...
	  public static Word[] addToArray(Word[] wa, Word newW){
	        Word[] newWa;
	        if ((newW == null) || (isMembOfArray(newW, wa))) return wa;
	        else {
	            int len = 0;
	            if (wa != null) len = wa.length;
	            newWa = new Word[len+1];
	            newWa[len] = newW;
	            for (int i=0; i < len; i++) newWa[i] = wa[i];
	        }
	        return newWa;
	  }
	...
	// static method for inserting a word at the front of an array:
	  ...
	  public static Word[] addArrayElement (Word elt, Word[] array) {
	    int oldLen = 0;
	    if (array != null){oldLen = array.length;}
	    Word [] newArray = new Word[oldLen + 1];
	    newArray[0] = elt;
	    for (int i = 0; i < oldLen; i++) {newArray[i + 1] = array[i];}
	    return newArray;
	  }
	...
	public static int encodeWords(BitBuffer bb, Word[] wds){
	      ...
	      if (n == 1){
	          ...
	          for (int i=0; i < n; i++){
	              ...
	              if (wi <= 0) wi = wds[i].assignWordIndex();
	              ints[i] = wi;
	          }
	...
	public static int encodeWordP(BitBuffer bb, Word w){
	      ...
	      if (windex < 1) windex = w.assignWordIndex();
	...
	public static int encodeValues(BitBuffer bb, Value[] vals){
	      ...
	      if ((vals == null) || ((n = vals.length) == 0)){
	          ...
	          for (int i=0; i < n; i++) nBits += encodeValue(bb, vals[i]);
	          return nBits + 1;
	      }
	  }
	...
	public static int encodeValue(BitBuffer bb, Value v){
	      ...
	      if ((v == null) || (v.listp()))
	          ...
	          Lexicon.log.debug(Lexicon.logTag, 10, "encodeValue can't do phrases!");
	      else nBits = encodeAtomVal(bb, (Atom)v);
	      return nBits;
	  }
	...
	public static int encodeList(BitBuffer bb, List lst){
	      ...
	      Value[] lvals = null;
	      if (lst != null) lvals = lst.contents;
	...
	public static int encodeCategoryP(BitBuffer bb, Category c){
	      int cindex = c.index;
	      if (cindex < 1) cindex = c.assignCategoryIndex();
	...
	/* methods to interface Lexicon objects to the BitBuffer */
	  ...
	  public static int encodeAtoms(BitBuffer bb, Atom[] atms){
	      ...
	      for (int i=0; i < n; i++) {
	          if (atms[i] == null)
	               Lexicon.log.debug(Lexicon.logTag, 10, "null atom in element " + i +
	...
	public static int encodeAtomP(BitBuffer bb, Atom a){
	      int aindex = a.index;
	      if (aindex < 1) aindex = a.assignAtomIndex();
