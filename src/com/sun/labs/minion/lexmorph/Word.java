   ...
   //DISKOUTDATED for dirty
	...
	wordstring = str;
      ...
      if (LexiconUtil.startsLikeNumber(str)){ //see if this word has a numericalNumber value
          try {testNum = new Double(wordstring);
          }
          catch (NumberFormatException e) {
          }
      }
      if ((testNum != null) || (LexiconUtil.senseNameStringp(str)) || (LexiconUtil.compoundStringp(str))) {
    ...
    public Word markNumWord(Number num){
      ...
      if (we == null) we = new WordEntry(this.lexicon, this);
      we.sensenamep = null;
      we.numericalValue = num;
      we.numeralp = true;
      if (num instanceof Integer)
            we.noPenaltyCategories = new Category[] {this.lexicon.integerCategory};
      else
    ...
    public boolean wordp() {return true;}
    ...
    public boolean ownsWordEntry(){
      ...
      return (wordEntry != null || bbuf != null || (index & Lexicon.MARKEDINDEX) == 0);
    }
    ...
    public WordEntry getWordEntry(){ //pmartin 31aug01
      ...
      if (localbb != null){
        ...
        this.lexicon.decodeWord(mybb, (index & Lexicon.WORDINDEXMASK), Lexicon.UNPACKALL);
    ...
    public boolean crush() {
      if (!hasWordEntry()) return false;
      if (this.lexicon.productionFlag) filterProps(this.lexicon.productionUnneeded);
//       String idxTags = "";
...
//       }
    ...
    public void filterProps(HashSet badprops){ // hacked for concurrency
       ...
       if (we != null) props = we.properties;
       if (props != null){
    ...
    protected int assignWordIndex (){
      if ((this.lexicon.highWordIndex + 1) >= this.lexicon.IndexWordLimit) this.lexicon.incFixWordIndex();
      int newIdx = this.lexicon.highWordIndex++;
      this.lexicon.valueIndexTable[newIdx] = this;
      Lexicon.log.debug(Lexicon.logTag, 10, "assignWordIndex " + newIdx + " for " +
                printEntryString());
    ...
    public BitBuffer encode(BitBuffer bb){ // pmartin 17sep01
      if ((index & Lexicon.WORDINDEXMASK) <= 0) assignWordIndex();
    ...
    public WordEntry reloadDiskWord(){//  31aug01
      ...
      WordEntry we = null;
      if ((windex & (Lexicon.NODISKINDEX | Lexicon.NOWORDENTRY)) != 0) return null;
         ...
         Word ww = (Word)this.lexicon.reloadBinObj(windex, this.lexicon.binLexRAF);
         we = ww.wordEntry;
      }else {
    ...
    public void setNumericalValue(Number numval) {
      ...
      if (we.numeralp) {
          if (numval.equals(we.numericalValue)) {
           ...
           Lexicon.log.debug(Lexicon.logTag, 10, "Warning: denied attempt to change value of " +
                             "a numeral number ->" +
    ...
    public Number findNumericalValue() { //  pmartin 22 dec 99
      ...
      if (numv != null)  return numv;
           ...
           Lexicon.log.debug(Lexicon.logTag, 10, "*****Broken word " + this.toString() +
    ...
    public Word normalizeIfNumber () { //pmartin 26apr01
      ...
      WordEntry we = getWordEntry();
      if (we == null) return this;
      Number nv = we.numericalValue;
      if ((nv == null) || (we.numeralp))
        return this;
      else return this.lexicon.makeWord(nv);
    }
    ...
    public boolean generalNumberp() { // added PMartin for LispATN needs .. 2 July 99
      ...
      else return (we.numericalValue != null) ;
    }
    ...
    public Word makeKnownWord(){ //force recursive installation
       if (Lexicon.authorFlag && Lexicon.wordTrace)
           Lexicon.log.debug(Lexicon.logTag, 10, "Lexicon: makeKnownWord " + printEntryString());
       Word knownWord = this.lexicon.getWord(this.wordstring);
       if (knownWord != null){
           boolean same = (knownWord == this);
           if (Lexicon.authorFlag && Lexicon.wordTrace){
               if (same) Lexicon.log.debug(Lexicon.logTag, 10, " ** was already in");
               else Lexicon.log.debug(Lexicon.logTag, 10, "*** being ignored for known " +
                              knownWord.printEntryString());
           }
       }else {
           if (Lexicon.authorFlag && Lexicon.wordTrace)
                Lexicon.log.debug(Lexicon.logTag, 10, " ** being installed");
           this.morphCache = null; // clear morphCache if there is one // waw 02-18-05
           this.lexicon.installWord(this, this.wordstring);
       }
       if (Lexicon.authorFlag && Lexicon.wordTrace)
            Lexicon.log.debug(Lexicon.logTag, 10, " and forcing parts known\n");
       makeKnownParts();
       return this;
    }
    ...
    public Word makeKnownWordInner(){  // pmartin 26sep01
       if (Lexicon.authorFlag && Lexicon.wordTrace)
            Lexicon.log.debug(Lexicon.logTag, 10, "Lexicon: makeKnownWordInner " + printEntryString());
       Word knownWord = this.lexicon.getWord(this.wordstring);
       if (knownWord != null){
           boolean same = (knownWord == this);
           if (Lexicon.authorFlag && Lexicon.wordTrace){
    ...
    public List makeKnownWordsList(List wdslst){
      ...
      if ((wdslst != null)  && (wdslst.length() > 0))
           for (int i=0; i < wdslst.contents.length; i++){
               ...
               else Lexicon.log.debug(Lexicon.logTag, 10, "warning -- discarding element num "
                              + i + " of wdlist " + wdslst.toString() +
                              " is not a word.");
      }
      return wdslst;
    }
    ...
    public Word makeKnownParts(){ // make scratch words inside it real
       ...
       //
       WordEntry we = getWordEntry();
       if (we == null) return this;  // nothing to fixup
       ...
       if (we.sensenamep != null) we.sensenamep[1] =
    ...
    public Word mergeWord(Word other){ //pmartin 5dec01
       ...
       if (owe == null) return this;  // nothing to merge
       dontPurge = dontPurge || other.dontPurge;
       dirty(); // we will make changes
       Category[] newCats;
       we.noPenaltyCategories =
    	   LexiconUtil.mergeArrays(we.noPenaltyCategories, owe.noPenaltyCategories);
       HashSet done = new HashSet();
       LexiconUtil.hashArray(done, we.noPenaltyCategories);
       newCats =
    	   LexiconUtil.mergeExcluding(done, we.penalty1Categories, owe.penalty1Categories);
       LexiconUtil.hashArray(done, newCats);
       we.penalty1Categories = newCats;
       newCats =
    	   LexiconUtil.mergeExcluding(done, we.penalty2Categories, owe.penalty2Categories);
       LexiconUtil.hashArray(done, newCats);
       we.penalty2Categories = newCats;
       newCats =
    	   LexiconUtil.mergeExcluding(done, we.penalty3Categories, owe.penalty3Categories);
       LexiconUtil.hashArray(done, newCats);
       we.penalty3Categories = newCats;
       we.rootCategories = LexiconUtil.mergeArrays(we.rootCategories, owe.rootCategories);
       ...
       we.inflectionCodes = LexiconUtil.mergeArrays(we.inflectionCodes, owe.inflectionCodes);
       ...
       if (owe.properties != null){
           ...
           while (temp.hasMoreElements()) {
               ...
               else we.putdict(key, this.lexicon.mergeValues(newVal, othVal));
           }
       }
       if (owe.numeralp){
       ...
       if (owe.sensenamep != null) we.sensenamep = owe.sensenamep;
    ...
    public boolean loopingProperty (Word vword){
       ...
       if (vword == null)
           return false;
       if (vword == this) return true; // it actually happens!
       WordEntry we = getWordEntry();
       if (we == null) return false;
       if (vword.wordOrSensewordMemb(we.ikoParents) ||
           ...
           vword.wordOrSensewordMemb(getVariants())) return true;
       if (we.sensenamep == null) return false;
       Word sw = getSenseWord();
       if (vword == sw) return true;
    ...
    public boolean wordOrSensewordMemb(Word[] ups) {
       if (ups != null){
           if (LexiconUtil.isMembOfArray(this, ups)) return true;
  ...
  //  pmartin 9may01
    ...
    public List getCapcodes(){
       WordEntry we = getWordEntry();
       if (we == null) return null;
       else return we.capcodes;
    }

    public Value[] getCompoundOf(){
            WordEntry we = getWordEntry();
            if (we == null) return null;
            else return we.compoundOf;
    }
    ...
    public Atom[] getInflectionCodes(){
            WordEntry we = getWordEntry();
            if (we == null) return null;
            else return we.inflectionCodes;
    }
    ...
    public Atom[] addInflectionCode(Atom newCode){ //pmartin 17july01
        ...
        if (LexiconUtil.memb(newCode, oldInflCodes) >= 0){
          ...
          int oldLen = 0;
          if (oldInflCodes != null) oldLen = oldInflCodes.length;
          Atom[] newInflCodes = new Atom[oldLen+1];
          for(int i=0; i < oldLen; i++) newInflCodes[i] = oldInflCodes[i];
    ...
    public Atom[] addInflectionCodes(Atom[] newCodes){
        ...
        for (int i=0; i < oldCodes.length; i++) hcode.add(oldCodes[i]);
        for (int i=0; i < newCodes.length; i++) hcode.add(newCodes[i]);
        int newC = hcode.size();
    ...
    public Word[] addIkoParents(List newParents){
      ...
      Value[] newvs = newParents.contents;
       ...
       for (int i=0; i < newvs.length; i++) newps[i] = (Word)newvs[i];
    ...
    public Word[] getRoots(Category cat){ //pmartin 16may01
        ...
        if (we == null) return null;
        Word[] wroots = we.roots;
        if (wroots == null) return null; //waw 31may01
        int wrlen = wroots.length;
        if (wrlen == 0) return null;
    ...
    public Word[] removeRoots(Word[] badRoots){ // pmartin 25mar04
        ...
        if (we == null) return null;
        Word[] wroots = we.roots;
        if (wroots == null || wroots.length == 0) return null;
        if (badRoots == null || badRoots.length == 0) return wroots;
        boolean  changed = false;
        HashSet hroots = new HashSet(wroots.length);
        for (int i=0; i < wroots.length; i++) hroots.add(wroots[i]);
        for (int i=0; i < badRoots.length; i++)
            if (hroots.contains(badRoots[i])){
                hroots.remove(badRoots[i]);
                changed = true;
            }
        if (!changed) return wroots;
    ...
    public Word[] addCutVarOf(Word newa){
        ...
        else  newAbbrvs = LexiconUtil.addToArray(LexiconUtil.valueWords(old), newa);
    ...
    public boolean variantsLinked(){
       WordEntry we = getWordEntry();
       if (we == null) return false;
       else return (we.getdict(this.lexicon.variantsLinkedAtom) == this.lexicon.trueAtom);
    }
    ...
    public Word[] addNicknameOf(Word newa){
        ...
        we.putdict(this.lexicon.nicknameOfAtom, new List(this.lexicon, newAbbrvs));
        return newAbbrvs;
    }
...
//                           newSenseName.printEntryString());
       ...
       if (abrvof.sensenamep()){
          awsns = new Word[] {makeVariantSenseName(abrvof, null)};
           ...
           for (int i=0; i < acats.length; i++){
               ...
               if (acats.length == 1)
                   fullw = abrvof;
               else fullw = abrvof.makeSenseName(acats[i]);
    ...
    public Word[] getInverseVariantLinks(){
        if (getWordEntry() == null) return null;
        HashSet av = null;
        boolean empty = true;
        Word[] avars = getAllVariants();
        if (avars != null) empty = false;
        Word[] ivars = getVariants();
        if (ivars != null) empty = false;
        Word[] msps = getMisspellings();
        if (msps != null) empty = false;
        Word[] nkns = getNicknames();
        if (nkns != null) empty = false;
        Word[] abrvs = getAbbrevs();
        if (abrvs != null) empty = false;
        if (empty) return null;
        av = new HashSet();
        if (avars != null)
            for (int i=0; i < avars.length; i++) av.add(avars[i]);
        if (ivars != null)
            for (int i=0; i < ivars.length; i++) av.add(ivars[i]);
        if (msps != null)
            for (int i=0; i < msps.length; i++) av.add(msps[i]);
        if (nkns != null)
            for (int i=0; i < nkns.length; i++) av.add(nkns[i]);
        if (abrvs != null)
            for (int i=0; i < abrvs.length; i++) av.add(abrvs[i]);
    ...
    public void makeVariantLinks(){
       WordEntry we = getWordEntry();
       if ( we == null || we.getdict(this.lexicon.variantsLinkedAtom) == this.lexicon.trueAtom)
            return;
       Word[] vsw;
    ...
    public Word[] addEntails(Word newe){
        ...
        newe.makeWordEntry().putdict(this.lexicon.entailedbyAtom,new List(this.lexicon, neweb));
        return newEntails;
    }
    ...
    public Word[] addFalseRoot(Word newfrt){
        ...
        we.putdict(this.lexicon.falseRootAtom, new List(this.lexicon, newFalseRoots));
        return newFalseRoots;
    }

    public Word[] applyFalseRoot(Word newfrt){// pm 25mar04
       if (Lexicon.authorFlag && Lexicon.debug)
            Lexicon.log.debug(Lexicon.logTag, 10, "Lexicon: addFalseRoot " + newfrt.wordstring +
    ...
    public Word[] getMisspellingOf(){
        WordEntry we = getWordEntry();
        if (we == null) return null;
        else return LexiconUtil.valueWords(we.getdict(this.lexicon.misspellingOfAtom));
    }

    public Word[] getMisspellings(){
        WordEntry we = getWordEntry();
        if (we == null) return null;
        else return LexiconUtil.valueWords(we.getdict(this.lexicon.misspellingsAtom));
    }

     public Word[] addMisspellingOf(Word newm){
        WordEntry we = makeWordEntry();
        Word[] newMsplofs = LexiconUtil.addToArray(getMisspellingOf(), newm);
        we.putdict(this.lexicon.misspellingOfAtom, new List(this.lexicon, newMsplofs));
        return newMsplofs;
    }
    ...
    public Word[] addMisspellings(Word newm){
        ...
        we.putdict(this.lexicon.misspellingsAtom, new List(this.lexicon, newMspls));
        return newMspls;
    }
    ...
    public Word[] getSenseParents(){
        WordEntry we = getWordEntry();
        if (we == null) return null;
        else return we.getSenseParents();
    }
    ...
    public boolean punctuationp(){ //pmartin 25Aug99
      ...
      //   return (isPunctCat || ikoPunct);
        WordEntry we = getWordEntry();
        if (we == null) return false;
        else return we.isFormOfCat(this.lexicon.punctCategory);
    }
    ...
    public boolean isKnownWord(){ //pmartin 25Aug99
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return ( ! (we.isFormOfCat(this.lexicon.unknownCategory)) &&
                    ! this.guessedWordp());
    }
    ...
    public boolean guessedNamep (){ //pmartin 21dec00
        WordEntry we = getWordEntry();
        if (we == null) return false;
        else return (we.getdict(this.lexicon.guessedNameAtom) != null);
    }

    public List guessedNameIndex (){ //pmartin 21dec00
        WordEntry we = getWordEntry();
        if (we == null) return null;
        else return (List)(we.getdict(this.lexicon.guessedNameAtom));
    }
    ...
    public void addGuessedWordMark(){
        makeWordEntry().putdict(this.lexicon.authorAtom, this.lexicon.guessedAtom);
    }

    public void addMorphServerWordMark(){
        makeWordEntry().putdict(this.lexicon.authorAtom, this.lexicon.morphServerAtom);
    }

    public void markNotYetMorphed(){ //pmartin 28jun01 support morphRoot
        makeWordEntry().putdict(this.lexicon.morphStatusAtom, this.lexicon.notYetMorphedAtom);
    }
    ...
    public boolean testNotYetMorphed(){// true iff flagged as not yet
      ...
      WordEntry we = getWordEntry();
      if (we == null)  return false; //
      else return (we.getdict(this.lexicon.morphStatusAtom) == this.lexicon.notYetMorphedAtom);
    }
    ...
    public void markTransmitted(){ //pmartin 4aug00 for concept Store
        makeWordEntry().putdict(this.lexicon.transmitFlagAtom, this.lexicon.trueAtom);
    }

    public boolean transmittedp(){ // pmartin 4aug00 for the conceptStore
        WordEntry we = getWordEntry();
        if (we == null) return false;
        else return (we.getdict(this.lexicon.transmitFlagAtom) != null);
    }
    ...
    public void markDerivedVerb(Word rootv, Atom form, Atom tense){
      ...
      if (tense != null) we.markDict(this.lexicon.tenseAtom, tense, true);
      if ((form == this.lexicon.pastpartAtom) || (form == this.lexicon.prespartAtom) )
    ...
    public void mergeIcat(Category nonIcat, Category icat){
        ...
        if (penalty > 3) penalty = 0;
        addWordCategory(icat, penalty);
    }

    public Lexicon lexicon() {return this.lexicon;} //the lexicon this is in
    ...
    public boolean hasFeature (Atom feature) { /* to replace check-dict */
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return feature.isInArray(we.features);
    }

    public boolean hasPrefix (Word[] list) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return LexiconUtil.intersectp(list, we.prefixes);
    }

    public boolean hasSuffix (Word[] list) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return LexiconUtil.intersectp(list, we.suffixes);
    }
    ...
    public boolean hasCommonCat () { // for word
     ...
     if (Lexicon.authorFlag && Lexicon.debug)
         Lexicon.log.debug(Lexicon.logTag, 10, wordstring + ".hasCommonCat() " + hcc);
      return hcc;
    }
    ...
    public int getCatPenalty (Category c){ //pmartin 17may01
      WordEntry we = getWordEntry();
      if (we == null) return 4; // the bogus penalty flag
      else return we.getCatPenalty(c);
    }
    ...
    public boolean someSenseIsKindOf (Word parent) {// pmartin 15may01
      WordEntry we = getWordEntry();
      if (we == null) return false;
      if (isKindOf(parent)) return true;  // pmartin 9aug01
      Word[] subs = we.subsenses;
      if (subs == null) return false;
      else for (int ii=0; ii < subs.length; ii++)
        if (subs[ii].isKindOf(parent)) return true;
      return false;
    }
    ...
    public boolean isNonnameFormOfCat (Category c) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return we.isNonnameFormOfCat(c);
    }
    ...
    public boolean isNonnameFormOfCat (Category[] cc) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return we.isNonnameFormOfCat(cc);
    }
    ...
    public boolean hasNonnameCat () {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return (we.isNonnameFormOfCat(we.noPenaltyCategories) ||
    ...
    public boolean isNonpenaltyFormOfCat (Category c) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return c.subsumesOneOf(we.noPenaltyCategories);
    }

    public boolean isStrictlyProperNoun () { /* to replace is-only-npr */
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return (we.isFormOfCat(this.lexicon.nprCategory) &&
    ...
    public boolean isPenaltyFormOfCat (Category c) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return we.isPenaltyFormOfCat(c);
    }

    public boolean isPenaltyFormOfCat (Category c, int penalty) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return we.isPenaltyFormOfCat(c, penalty);
    }

    public boolean isRootOfCat (Category c) {
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return we.isRootOfCat(c);
    }
    ...
    public boolean sensenamep () {
     WordEntry we = getWordEntry();
     if (we == null) return false;
     else return (we.sensenamep != null);
    }
    ...
    public Category getSenseCat () {
      ...
      if (sen != null) return (Category)sen[0];
      else return null;
    }
    ...
    public String getSenseTailString () {
     ...
     if (we == null) return null;
     Object[] sen = we.sensenamep;
     if (sen != null) return (String)sen[2];
     else return null;
    }
    ...
    public Word makeSenseName (Category c, String tail) {
      Category useCat = c;
      if (useCat == null) useCat = this.lexicon.wordCategory;
      if (c == this.lexicon.nounCategory && isFormOfCat(this.lexicon.nmCategory))
          useCat = this.lexicon.ncCategory;
      else if (c == this.lexicon.verbCategory){
          if (isFormOfCat(this.lexicon.vtCategory) && !(isFormOfCat(this.lexicon.viCategory)))
              useCat = this.lexicon.vtCategory;
          else if (isFormOfCat(this.lexicon.viCategory) && !(isFormOfCat(this.lexicon.vtCategory)))
    ...
    public Word makeSenseName (Category c, Word parentKind){
       ...
       else return makeSenseName(c, parentKind, null);
    }
...
//                            vsWord.printEntryString());
    ...
    public Word makeSenseNameIfNeeded (Category c, Word pKind, String infl){
      ...
      if (isFormOfCat(c) && isAmbiguous()){
          ...
          if (pKind != null) addIkoParent(pKind);
      }
      return sw;
    }
   ...
   public Word[] makeSenseNamesIfNeeded (Category[] cs){
    ...
    public boolean hasRoot(Word rt){ //pmartin hack 15may01
      WordEntry we = getWordEntry();
      if (we == null) return false;
      else return LexiconUtil.isMembOfArray(rt, we.roots);
    }
      ...
      if (Lexicon.authorFlag)
        if (debugOntology){
          ...
          while (seenit.hasNext())
            seenStr = seenStr + " " + ((Word)seenit.next()).printString();
    ...
    public Word[] nonRootParents (boolean nonames) {
      ...
      if (Lexicon.authorFlag)
        if (trace)
          Lexicon.log.debug(Lexicon.logTag, 10, "in nonRootParents of " + this.printString() +
      ...
      if (Lexicon.authorFlag)
        if (trace)
          System.out.println("in nonRootParents of " + this.printString() +
                             " iko are:" + LexiconUtil.printStringArray(we.ikoParents));
      ...
      if (Lexicon.authorFlag)
        if (trace){
          String senseOf = "null";
          if (sensParents != null) senseOf = sensParents.printString();
          System.out.println("in nonRootParents of " + this.printString() +
                             " senseOf are:" +  senseOf);
        }
      if (sensParents != null){
...
//                         personStopperHash.size());
      ...
      if (inrp == 0) return null;
      if (inrp == nrpSize) return maybeParents;
      Word[] parents = new Word[inrp];
      System.arraycopy(maybeParents, 0, parents,0, inrp);
      if (Lexicon.authorFlag)
        if (trace) System.out.println("nonRootParents of " + this.printString() +
    ...
    public Word[] allVariantParents(){
      ...
      if (vars != null)
          for (int i=0; i < vars.length; i++) avp.add(vars[i]);
      if (abrvs!= null)
          for (int i=0; i < abrvs.length; i++) avp.add(abrvs[i]);
      int avpSize = avp.size();
      if (avpSize == 0) return null;
    ...
    public Word[] getIkoParents(boolean useSubSenses){
      WordEntry we = getWordEntry();
      if (we == null) return null;
      else return we.getIkoParents(useSubSenses);
    }
    ...
    public Word[] getDirectRoots () {
      ...
      if (!(isInArray(myRoots))) return myRoots;
      else if (myRoots.length == 1) return null;
      else {
        int unselfn = myRoots.length - 1;
        Word[] unself = new Word[unselfn];
        int j = 0;
        for (int i=0; i < myRoots.length; i++)
          if (this != myRoots[i]) unself[j++] = myRoots[i];
        return unself;
      }
    }
...
//                      " and proots = " + toStringArray(proots));
    ...
    public Word[] getPastRoots () {
     WordEntry we = getWordEntry();
     if (we == null) return null;
     else if (we.isFormOfCat(this.lexicon.pastCategory)) return we.roots;
     else return null;
    }
    ...
    public boolean subsumesOneOf (Word[] words) {
      ...
      while (i < words.length) {
        if (this.subsumesWord(words[i])) return true;
        i++;
      }
      return false;
    }
    ...
    public boolean subsumesWord (Word word, HashSet triedWords) {
      ...
      if (triedWords.contains(word)) return false;
      triedWords.add(word);
      WordEntry we = word.getWordEntry();
      if (we == null) return false;
      Word[] wdikos = we.ikoParents;
      if (wdikos == null) return false;
      for (int i=0; i < wdikos.length; i++) {
        if (this.subsumesWord(wdikos[i], triedWords)) return true;
      }
      return false;
    }
   ...
   //waw: approximate test
    public boolean wordIsEmpty(){ //pmartin 4sep01
      WordEntry we = getWordEntry();
      if (we == null) return true;
      /* no categories at all means the word is a "loose end"  or contraction*/
      else return ((we.noPenaltyCategories == null) &&
    ...
    public Category[] setWordCategories (String str, int penalty) {
      ...
      if (cats != null) {
        WordEntry we = makeWordEntry();
        if (penalty > 2) we.penalty3Categories = cats;
        else if (penalty > 1) we.penalty2Categories = cats;
        else if (penalty > 0) we.penalty1Categories = cats;
        else we.noPenaltyCategories = cats;
        return cats;
      }
      else return null;
    }
    ...
    public Value getFirstVal(Atom feat){ // shrunk pmartin 01aug01
      WordEntry we = getWordEntry();
      if (we == null) return null;
      else return LexiconUtil.firstVal(we.getdict(feat));
    }
    ...
    public Word[] getFirstDecomposition(){ // pmartin 22may01 + 24july01
      ...
      if ((compOf == null) || (compOf.length == 0))
        ...
        Word[] fw = new Word[cl];
        for (int i=0; i < cl; i++) fw[i] = (Word)compOf[i];
        return fw;
      }else if (compOf[0].listp()){
        Value[] con = ((List)compOf[0]).contents;
        int cl = con.length;
        Word[] fw = new Word[cl];
        for (int i=0; i < cl; i++) fw[i] = (Word)con[i];
        return fw;
      }else{
    ...
    public Word[] getFirstDecomposition(Atom feat){ // pmartin 15may01
      ...
      else val = this.getdict(feat);
      if (val == null) return null;
      if (val.listp()) {
        Value[] con = ((List) val).contents;
        if ((con == null) || (con.length == 0)) return null;
        if (con[0].listp())
          return (Word[])((List)con[0]).contents;
        else return (Word[])con;
      }
      else return null;
    }
    ...
    public Word restWords(){
      ...
      if ((rws == null) || (rws.length < 2))
        ...
        for (int i=0; i < rwl; i++) rrw[i] = rws[i+1];
      ...
      if (dangerousChars.length > 0)
            for (int i=0; i < dangerousChars.length; i++)
                if (wordstring.indexOf(dangerousChars[i]) != -1)
                    return this.lexicon.openQuoteStr + wordstring + this.lexicon.closeQuoteStr;
      return wordstring;
    }
