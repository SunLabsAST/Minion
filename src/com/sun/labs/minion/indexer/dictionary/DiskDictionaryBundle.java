...
//                    }
    ...
    public float getDocumentVectorLength(int docID) {
        return getDocumentVectorLength(docID, Field.DocumentVectorType.CASED);
    }
    ...
    public void normalize(int[] docs, float[] scores, int n, float qw) {
        normalize(docs, scores, n, qw, Field.DocumentVectorType.CASED);
    }
        ...
        for(DictionaryType type : DictionaryType.values()) {
            ...
            switch(type) {
                ...
                case CASED_VECTOR:
                    ...
                    // tokens dictionary.
                    encoder = new StringNameHandler();
                    mergeState.postIDMaps = entryIDMaps[DictionaryType.CASED_TOKENS.
                            ordinal()];
                ...
                case UNCASED_VECTOR:
                    encoder = new StringNameHandler();
                    mergeState.postIDMaps = entryIDMaps[DictionaryType.UNCASED_TOKENS.
                            ordinal()];
                    idStarts = mergeState.fakeStarts;
                    mergingDocEntries = true;
                    break;
...
//                    Level.INFO);
                        ...
                        for(int k = 0; k < n; k++) {
                            ...
                            try {
                                ...
                                logger.log(Level.SEVERE,
                                        ...
                                        bundles[i].dicts[DictionaryType.CASED_VECTOR.
        ...
        if(mergeState.info.hasAttribute(FieldInfo.Attribute.INDEXED)
            ...
            for(DocumentVectorType vecType : DocumentVectorType.values()) {
                DictionaryType tokenDictType = DocumentVectorType.getDictionaryType(vecType);
                
                long mdp = mergeHeader.dictionaryOffsets[tokenDictType.ordinal()];
                if(mdp < 0) {
                    mergeHeader.vectorLengthOffsets[vecType.ordinal()] = -1;
                    continue;
                }
                try {
                    calculateDocumentVectorLengths(mergeState, mergeHeader,
                                                   vecType, exemplar.
                            getTokenPostingsType(), fieldDictOut, mdp, mPostRAF);
                } catch(RuntimeException ex) {
                    logger.log(Level.SEVERE, String.format(
            ...
            if(logger.isLoggable(Level.FINER)) {
                dw.stop();
                logger.fine(String.
                        format("Calculating doc vec lens took %.2fms", dw.
                        getTimeMillis()));
                dw.reset();
            }
        }
    ...
    /**
     * Helper to calculate the document vector lengths for a given set of document
     * vectors.
     * 
     * @param mergeState
     * @param mergeHeader
     ...
     * @throws IOException 
     */
    private static void calculateDocumentVectorLengths(MergeState mergeState,
                                                       FieldHeader mergeHeader,
                                                       Field.DocumentVectorType vectorType,
                                                       Postings.Type postingsType,
                                                       DictionaryOutput dictOut,
                                                       long dictPosition,
                                                       RandomAccessFile[] mPostRAF) throws IOException {
        WriteableBuffer vlb = mergeState.partOut.getVectorLengthsBuffer();
        mergeHeader.vectorLengthOffsets[vectorType.ordinal()] = vlb.position();
        dictOut.position(dictPosition);
        DiskDictionary<String> newDict =
                new DiskDictionary<String>(new EntryFactory(postingsType),
        ...
        DocumentVectorLengths.calculate(mergeState.info,
                                        ...
                                        mergeState.partOut.getMaxDocID(),
                                        mergeState.manager,
                                        (DictionaryIterator<String>) newDict.
                iterator(),
                                        vlb,
                                        mergeState.manager.getTermStatsDict(),
                                        Field.DocumentVectorType.
     ...
     * @param termStatsHeader the header for the term statistics where we will put 
     * dictionary offsets.
     * @param termStatsDictOut the place where the term stats dictionary will
     * be written.
    ...
    public static void calculateTermStats(DiskDictionaryBundle[] bundles,
        ...
        if(bundles.length == 1) {
            ...
            for(TermStatsType termStatsType : TermStatsType.values()) {
                DictionaryType dictType = TermStatsType.getDictionaryType(termStatsType);
                DiskDictionary dict = bundles[0].getDictionary(dictType);
                if(dict != null) {
            ...
            for(TermStatsType termStatsType : TermStatsType.values()) {
                DictionaryType dictType = TermStatsType.getDictionaryType(
                        termStatsType);
                ...
                for(int i = 0; i < bundles.length; i++) {
                    ...
                    dicts[i] = bundles[i].getDictionary(dictType);
                    if(dicts[i] != null) {
                        nFound++;
                    }
                }
                ...
                int nWritten = calculateMultipleDictionaryTermStats(info, dicts,
                                                                    termStatsDictOut);
                if(nWritten > 0) {
                    termStatsHeader.addOffset(info.getID(), termStatsType,
                                              offset);
                }
            }
        }
    ...
    public void calculateVectorLengths(PartitionOutput partOut) throws java.io.IOException {
        ...
        for(DocumentVectorType vectorType : DocumentVectorType.values()) {
            DictionaryType dictType = DocumentVectorType.getDictionaryType(vectorType);
            DiskDictionary<String> termDict = (DiskDictionary<String>) getDictionary(dictType);
            if(termDict != null) {
                header.vectorLengthOffsets[vectorType.ordinal()] = vectorLengthsBuffer.position();
    ...
    public DocumentVectorLengths getDocumentVectorLengths() {
        return getDocumentVectorLengths(Field.DocumentVectorType.UNCASED);
    }
